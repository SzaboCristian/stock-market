...
class PortfolioManagementAPI:
    ...
    def get_portfolio(user_id, portfolio_id=None, **kwargs) -> tuple:
        ...
        if 'test' not in kwargs and not PortfolioManagementAPI.check_user_exists(user_id):
        ...
        es_portfolio_documents = es_dbi.search_documents(config.ES_INDEX_PORTFOLIOS, query_body=es_query)
        if es_portfolio_documents and es_portfolio_documents.get('hits', {}).get('hits', []):
            return 200, {portfolio['_id']: portfolio['_source'] for portfolio in
                         es_portfolio_documents['hits']['hits']}, 'OK'
    ...
    def create_portfolio(user_id, allocations, **kwargs) -> tuple:
        ...
        try:
            portfolio = Portfolio(portfolio_json={
                'portfolio_name': kwargs.get('portfolio_name', 'user_portfolio_default'),
        ...
        es_dbi = ElasticsearchDBI.get_instance(config.ELASTICSEARCH_HOST, config.ELASTICSEARCH_PORT)
        portfolio_id = es_dbi.create_document(config.ES_INDEX_PORTFOLIOS, document=portfolio.get_data())
    ...
    def update_portfolio(user_id, portfolio_id, new_allocations, **kwargs) -> tuple:
        ...
        es_dbi = ElasticsearchDBI.get_instance(config.ELASTICSEARCH_HOST, config.ELASTICSEARCH_PORT)
        portfolio_document = es_dbi.get_document_by_id(config.ES_INDEX_PORTFOLIOS, _id=portfolio_id)
        ...
        try:
            portfolio = Portfolio(portfolio_json={
                'portfolio_name': portfolio_document["_source"].get("portfolio_name", None),
                'created_timestamp': portfolio_document["_source"].get("created_timestamp", None),
                'modified_timestamp': portfolio_document["_source"].get("modified_timestamp", None),
        ...
        if es_dbi.update_document(config.ES_INDEX_PORTFOLIOS, _id=portfolio_id, document=portfolio.get_data()):
            return 200, {'portfolio_id': portfolio_id}, 'OK'
