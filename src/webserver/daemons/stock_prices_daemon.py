...
def are_any_markets_open() -> bool:
    ...
    """
    return (
        datetime.datetime.today().weekday() not in [5, 6]
        and 8 <= datetime.datetime.now(tz=pytz.timezone("Europe/Bucharest")).hour <= 24
    )
...
def should_update_stock_prices() -> bool:
    if markets_closed_the_day_before():
        Logger.info(
            "Markets were closed the day before. No new price info to be found."
        )
        return False

    if are_any_markets_open():
        Logger.info(
            "Markets are still open, waiting to close in order to update prices."
        )
        return False
...
def stock_prices_updater_task() -> None:
    ...
    while True:
        ...
        if (
            not ticker_last_price_dates
            or time.time() - last_ticker_fetch_ts >= 3 * _ONE_DAY
        ):
            tickers = get_all_tickers(es_dbi)
            if tickers:
                ticker_last_price_dates = get_last_price_date_for_tickers(
                    tickers, es_dbi
                )
        ...
        for ticker in ticker_last_price_dates:
            next_date = datetime.datetime.strptime(
                ticker_last_price_dates[ticker], "%Y-%m-%d"
            )
            ...
            if next_date != yf_now_date:
                ...
                missing_prices = yf_get_historical_price_data_for_ticker(
                    ticker=ticker, start_date=next_date, end_date=yf_now_date
                )
                ...
                # set last date
                last_price_date = datetime.datetime.fromtimestamp(
                    missing_prices[-1]["date"]
                ).strftime("%Y-%m-%d")
                updated_ticker_last_price_dates[ticker] = last_price_date

                Logger.info(
                    "Got price history for [{}]: {} -> {}".format(
                        ticker, ticker_last_price_dates[ticker], last_price_date
                    )
                )
