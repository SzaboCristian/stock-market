...
class RoutePortfolio(Resource):
    ...
    @api.doc(
        params={
            "portfolio_id": api_param_query(required=False, description="Portfolio id"),
        }
    )
    @api.doc(responses={200: "OK", 404: "User not found. | Portfolio not found."})
    @api.doc(security="apiKey")
    @token_required
    def get(current_user) -> response:
        portfolio_id = get_request_parameter(
            name="portfolio_id", expected_type=str, required=False
        )
        return response(
            *PortfolioManagementAPI.get_portfolio(current_user.public_id, portfolio_id)
        )
    ...
    @api.doc(
        params={
            "portfolio_name": api_param_form(
                required=False, description="Portfolio name"
            ),
            "allocations": api_param_form(
                required=True,
    ...
    def post(current_user) -> response:
        portfolio_name = get_request_parameter(
            "portfolio_name", required=False, expected_type=str
        )

        allocations, msg = get_request_parameter(
            "allocations", required=True, expected_type=list
        )
        if not allocations:
            return response_400(msg)

        return response(
            *PortfolioManagementAPI.create_portfolio(
    ...
    @api.doc(
        params={
            ...
            "allocations": api_param_form(
                required=True, description="Portfolio allocations"
            ),
        }
    )
    @api.doc(
    ...
    def put(current_user) -> response:
        portfolio_name = get_request_parameter(
            name="portfolio_name", required=False, expected_type=str
        )

        portfolio_id, msg = get_request_parameter(
            name="portfolio_id", required=True, expected_type=str
        )
        if not portfolio_id:
            return response_400(msg)

        allocations, msg = get_request_parameter(
            name="allocations", required=True, expected_type=list
        )
        if not allocations:
            return response_400(msg)

        return response(
            *PortfolioManagementAPI.update_portfolio(
                user_id=current_user.public_id,
    ...
    def delete(current_user) -> response:
        portfolio_id, msg = get_request_parameter(
            name="portfolio_id", expected_type=str, required=True
        )
        if not portfolio_id:
            return response_400(msg)
        return response(
            *PortfolioManagementAPI.delete_portfolio(
                user_id=current_user.public_id, portfolio_id=portfolio_id
            )
        )
...
class RouteBacktest(Resource):
    ...
    @api.doc(
        params={
            ...
            "end_ts": api_param_query(
                required=False, description="End ts", default=int(time.time())
            ),
        }
    )
    @api.doc(
    ...
    def get(current_user) -> response:
        ...
        return response(
            *PortfolioManagementAPI.backtest_portfolio(
                user_id=current_user.public_id,
                portfolio_id=portfolio_id,
                start_ts=start_ts,
                ends_ts=end_ts,
