...
class RouteUsers(Resource):
    ...
    @api.doc(
        params={"public_id": api_param_query(required=False, description="User id")}
    )
    ...
    def get(current_user) -> response:
        ...
        public_id = get_request_parameter(
            "public_id", required=False, expected_type=str
        )
    ...
    def post() -> response:
        username, msg = get_request_parameter(
            name="username", expected_type=str, required=True
        )
        if not username:
            return response_400(msg)

        password, msg = get_request_parameter(
            name="password", expected_type=str, required=True
        )
        if not password:
            return response_400(msg)
        ...
        return response(
            *UsersManagementAPI.create_user(
    ...
    @api.doc(responses={201: "OK", 400: "Param <> is required", 404: "User not found."})
    @api.doc(security="apiKey")
    @token_required
    def put(current_user) -> response:
        ...
        public_id, msg = get_request_parameter(
            "public_id", required=True, expected_type=str
        )
        if not public_id:
            return response_400(msg)
    ...
    @api.doc(responses={200: "OK", 400: "Param <> is required", 404: "User not found."})
    @api.doc(security="apiKey")
    @token_required
