...
class RouteStocks(Resource):
    ...
    @api.doc(
        params={
            "ticker": api_param_query(required=False, description="Company ticker"),
            "company_name": api_param_query(required=False, description="Company name"),
            "sector": api_param_query(required=False, description="Sector"),
            "industry": api_param_query(required=False, description="Industry"),
            "tags": api_param_query(required=False, description="Tags"),
            "exchange": api_param_query(required=False, description="Stock Exchange"),
            "legal_type": api_param_query(required=False, description="Legal type"),
            "tickers_only": api_param_query(
                required=False,
    ...
    @api.doc(responses={200: "OK", 404: "No stocks found for specified filter."})
    def get() -> response:
        ticker = get_request_parameter(name="ticker", expected_type=str, required=False)
        company_name = get_request_parameter(
            name="company_name", expected_type=str, required=False
        )
        sector = get_request_parameter(name="sector", expected_type=str, required=False)
        industry = get_request_parameter(
            name="industry", expected_type=str, required=False
        )
        tags = get_request_parameter(name="tags", expected_type=str, required=False)
        exchange = get_request_parameter(
            name="exchange", expected_type=str, required=False
        )
        legal_type = get_request_parameter(
            name="legal_type", expected_type=str, required=False
        )
        tickers_only = get_request_parameter(
            name="tickers_only", expected_type=bool, required=False
        )

        return response(
            *StocksManagementAPI.get_stocks(
                ticker=ticker,
    ...
    @api.doc(
        params={
            "ticker": api_param_form(required=True, description="Company ticker"),
            "ticker_info": api_param_form(
                required=True, description="Updated information (json dict)"
            ),
        }
    )
    @api.doc(
    ...
    def put(current_user) -> response:
        ...
        ticker_info, msg = get_request_parameter(
            name="ticker_info", expected_type=dict, required=True
        )
        if not ticker_info:
            return response_400(msg)

        return response(
            *StocksManagementAPI.update_stock_info(
                ticker=ticker, updated_info=ticker_info
            )
        )
